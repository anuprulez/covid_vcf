...
def find_optimal_clusters(low_dimensional_features, number_iter=40, cluster_step=20, initial_clusters=50):
    clustering_perf = dict()
    for i in range(0, number_iter):
        print("Clustering iteration: {}/{}".format(str(i + 1), str(number_iter)))
        clustering_type = MiniBatchKMeans(n_clusters=initial_clusters, random_state=32)
...
def cluster(features, s_name_df, s_idx_df, var_name_df, var_pos_df, var_af_df, var_ref_df, var_alt_df, samples_name_idx, BOSTON_DATA_PATH, path_plot_df="data/test_clusters.csv"):
    ...
    scatter_df = pd.DataFrame(list(zip(s_name_df, s_idx_df, var_ref_df, var_pos_df, var_alt_df, var_af_df, clusters, var_name_df)), columns=["Sample", "Index", "REF", "POS", "ALT", "AF",  "Cluster", "annotations"], index=None)
